...
func dfs(grid [][]int, current, r, c int) []position {
	fmt.Printf("r: %d, c: %d\n", r, c)
	...
	total := []position{}
	total = append(total, dfs(grid, grid[r][c], r-1, c)...)
	total = append(total, dfs(grid, grid[r][c], r+1, c)...)
	total = append(total, dfs(grid, grid[r][c], r, c+1)...)
	total = append(total, dfs(grid, grid[r][c], r, c-1)...)
...
func part1() {
	...
	for _, th := range trailheads {
		...
		reachableNines = append(reachableNines, dfs(grid, 0, th.r-1, th.c)...)
		reachableNines = append(reachableNines, dfs(grid, 0, th.r+1, th.c)...)
		reachableNines = append(reachableNines, dfs(grid, 0, th.r, th.c-1)...)
		reachableNines = append(reachableNines, dfs(grid, 0, th.r, th.c+1)...)
